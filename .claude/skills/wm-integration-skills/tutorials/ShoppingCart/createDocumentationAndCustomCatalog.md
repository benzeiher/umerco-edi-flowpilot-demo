...
You must synchronize the signature metadata for the .flow file: `tutorials/ShoppingCart/existing-services-flow-script/applyDiscount.flow` into a staging Service Catalog.  

### Execution Instructions
1. Check for the existence of the file: `skills/flow/service-catalogs/staging/SHOPPING_CART.json`.
   - **If the file does not exist:** Dynamically create `SHOPPING_CART.json` in that directory. Seed it with the default catalog shell structure containing `"folder": "SHOPPING_CART"`, `"description"`, and a `"total_elements"` counter set to 0.
   - **If the file exists:** Open and parse it directly.
2. Extract the service structure names, input configurations, output configurations, and comment properties from the FSL file in `tutorials/ShoppingCart/existing-services-flow-script`.
3. Perform an **Upsert Mutation** against the `elements` array within `SHOPPING_CART.json` matching the catalog schema format:
...
4. Increment or update the value of the top-level `"total_elements"` property in `SHOPPING_CART.json` to accurately reflect the total number of services in the file.
5. If you do not have a comment or description for the service described by the FSL file or any of its signature elements, then try make an educated guess as to what the description should be based on the FSL logic. Try to keep the descriptions to one or two sentences.
...
**Mandatory Rule**: Every generated script file (e.g., `ServiceName.flow`) MUST have a corresponding Markdown documentation file (ServiceName.md) generated in the directory: `tutorials/ShoppingCart/existing-services-flow-script`.
**Naming**: The names of each markdown file should match the service name for the FSL being generated. E.g.: applyDiscount.md
...
- Markdown documentation file in `tutorials/ShoppingCart/existing-services-flow-script`
...
4. `skills/flow/service-catalogs/staging/SHOPPING_CART.json` has been created or updated successfully
